...
exports.handleReply = o=> {
    ...
    if (!category)return send(`STT không tồn tại!`);

    data[tid][category] = !status?true: false;
    send(`Đã ${!status?'bật': 'tắt'} chế độ vô hiệu hoá nhóm lệnh ${category}`);
    save();
};