...
module.exports.run = async ({ api, event }) => {
    ...
    if (autoTranslateThreads.has(threadID)) {
        ...
        api.setMessageReaction("✅", event.messageID, () => { }, true);
        api.sendMessage("Chọn chế độ dịch tự động:\n1. Tiếng Việt <-> Tiếng Anh\n2. Tiếng Nhật <-> Tiếng Việt", threadID, (err, info) => {
...
module.exports.handleReply = async ({ api, event, handleReply }) => {
    ...
    if (choice === '1') {
        ...
        api.sendMessage("Auto-translate đã được thiết lập để dịch giữa tiếng Việt và tiếng Anh.", threadID);
    } else if (choice === '2') {
        autoTranslateThreads.set(threadID, "ja-vi");
        api.sendMessage("Auto-translate đã được thiết lập để dịch giữa tiếng Nhật và tiếng Việt.", threadID);
    } else {
...
module.exports.handleEvent = async ({ api, event }) => {
    ...
    if (event.body.toLowerCase() === "autotrans off") {
        autoTranslateThreads.delete(threadID);
    ...
    request(encodeURI(`https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl=vi&dt=t&q=${message}`), (err, response, body) => {
        ...
        request(encodeURI(`https://translate.googleapis.com/translate_a/single?client=gtx&sl=${detectedLang}&tl=${targetLang}&dt=t&q=${message}`), (err, response, body) => {
