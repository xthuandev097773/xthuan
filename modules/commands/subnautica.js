...
module.exports.run = async function ({
    ...
    case '-r': {
        ...
        if (!existsSync(pathData)) {
            ...
            var msg = {body: "[ Game Câu Cá ]\n──────────────────\n✅ Đăng ký game thành công\n🏬 /cauca shop/-s: Để mua vật phẩm câu cá!", attachment: await this.subnautica()}
            return api.sendMessage(msg, threadID, messageID);
        } else return api.sendMessage({body: "[ Game Câu Cá ]\n──────────────────\n⚡ Bạn đã đăng ký game rồi!", attachment: await this.subnautica()}, threadID, messageID);
    }
    case 'shop':
    case '-s': {
        if (!existsSync(pathData)) {
            return api.sendMessage({body: "[ Game Câu Cá ]\n──────────────────\n🦈 Bạn chưa đăng ký tài khoản\n⚡ /cauca register/-r: Để đăng ký game!", attachment: await this.subnautica()}, threadID, messageID);
        }
        return api.sendMessage({body: "[ Cửa Hàng ]\n──────────────────\n1 » 💰 Mua vật phẩm\n2 » 💵 Bán vật phẩm câu được\n3 » ⚡ Nâng cấp/Sửa chửa vật phẩm\n──────────────────\n💬 Phản hồi tin nhắn này với lựa chọn của bạn!", attachment: await this.subnautica()}, threadID, (error, info) => {
    ...
    case 'help': {
            return api.sendMessage({body: "[ Hỗ Trợ Game ]\n──────────────────\n🦈 /cauca register/-r: Đăng ký game\n🏬 /cauca shop/-s: Cửa hàng câu cá\n🌊 /cauca custom/-c rod/custom locate: Lựa chọn khu vực câu cá\n🎒 /cauca bag/-b: Xem túi đồ", attachment: await this.subnautica()}, threadID, messageID);
        }
    default: {
