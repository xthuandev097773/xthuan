...
const pathApi = path.join(__dirname, "../../gojo/datajson/");
      ...
      if (args[0].toLowerCase() === "rm") {
        ...
        for (const fileIndex of fileIndices) {
          if (fileIndex >= 1 && fileIndex <= fileListArray.length) {
            ...
            api.setMessageReaction("✅", event.messageID, () => { }, true);
            send(`Đã xóa file ${selectedFile.fileName}`);
          } else {
            api.setMessageReaction("❎", event.messageID, () => { }, true);
            send(`Tên ${fileIndex} không hợp lệ`);
          }
        }
      } else if (args[0].toLowerCase() === "cr") {
        if (args.length === 1) {
          api.setMessageReaction("❎", event.messageID, () => { }, true);
          return send(`➣ Bạn cần nhập tên file để tạo!`);
        }
        ...
        for (const fileIndex of fileIndices) {
          if (fileIndex >= 1 && fileIndex <= fileListArray.length) {
            ...
            try {
              ...
              api.setMessageReaction("✅", event.messageID, () => { }, true);
              send(`➣ ${selectedFile.fileName}: ${mockyLink}`);
            } catch (error) {
              console.error(
              ...
              api.setMessageReaction("❎", event.messageID, () => { }, true);
              send("Đã xảy ra lỗi trong quá trình xử lý!");
            }
          } else {
            api.setMessageReaction("❎", event.messageID, () => { }, true);
            send(`Tên file ${fileIndex} không có `);
          }
        }
        ...
        for (const fileIndex of fileIndices) {
          if (fileIndex >= 1 && fileIndex <= fileListArray.length) {
            ...
            api.setMessageReaction("❎", event.messageID, () => { }, true);
            send(`Index ${fileIndex} không hợp lệ`);
          }
        }
      }
